...
export function install() {
  ...
  elms.forEach(elm => {
    var source = elm.src
    var radio = 0.8
    if(elm.attributes && elm.attributes.width && elm.attributes.width.nodeValue) radio = parseFloat(elm.attributes.width.nodeValue) / 100

    // update by 12302
    var paragraphParentNode = elm.parentNode;
    var clearfixDivExists = paragraphParentNode.querySelector('div.clearfix');
    // 如果不存在则插入
    if (!clearfixDivExists) {
      var newDiv = document.createElement('div');
      newDiv.className = 'clearfix';
      paragraphParentNode.appendChild(newDiv);
    }

    let options = Object.assign({}, defaultOptions, {}, parseOptions(elm.title))
    ...
    loader.imageLoaded = function () {
      ...
      let widthValue = loader.animator.width * radio
