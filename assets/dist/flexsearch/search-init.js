...
window.Million = { navigate: (url) => (window.location.href = url), prefetch: () => {} }
...
const fetchData = Promise.all([
    ...
    fetch("{{ $contentIndex.Permalink }}").then(data => data.json()),
])
.then(([{index, links}, content]) => ({index, links, content, }))
...
const displayResults = (term, finalResults, extractHighlight = false) => {
  ...
  const updateSelection = (index) => {
    // 移除当前选中的高亮
    const currentSelected = document.querySelector(".result-card.selected");
    if (currentSelected) {
      currentSelected.classList.remove("selected");
    }
    selectedIndex = index;
    // 高亮新的选中结果
    if (selectedIndex >= 0 && selectedIndex < finalResults.length) {
      const newSelected = document.getElementsByClassName("result-card")[selectedIndex];
      if (newSelected) {
  ...
  const handleKeydown = (event) => {
    switch (event.key) {
      ...
      case "ArrowDown":
        event.preventDefault();
        selectedIndex = Math.min(finalResults.length - 1, selectedIndex + 1);
        updateSelection(selectedIndex);
        break;
      case "Enter":
        event.preventDefault();
        if (selectedIndex >= 0 && selectedIndex < finalResults.length) {
          const selectedAnchor = document.getElementsByClassName("result-card")[selectedIndex];
          if (selectedAnchor) {
            redir(selectedAnchor.id, term);
          }
        }
        break;
    }
